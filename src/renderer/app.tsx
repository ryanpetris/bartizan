...
import { shortcutKey, type State } from '../shared';
import {
  api,
  store,
  render,
...
connect.onToggle(browser.syncNativeView);
...
function App() {
  ...
  useLayoutEffect(() => {
    const unsubscribe = api.onEvent((event) => {
      switch (event.type) {
        ...
        case 'browser-shortcut':
          ...
          if (event.action === 'focus-address') browser.focusAddress();
          else openConnection();
          break;
      }
    });
    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      const devTools =
        (key === 'f12' && !ctrl && !meta && !alt && !shift) ||
        (key === 'i' && ctrl && shift && !meta && !alt) ||
        (key === 'i' && meta && alt && !ctrl && !shift);
      if (devTools && store.selection?.kind === 'browser' && !dialogOpen()) {
      ...
      } else if (!event.shiftKey && key === 'l' && store.selection?.kind === 'browser' && !dialogOpen()) {
        event.preventDefault();
        event.stopPropagation();
