...
test('settings and profiles persist together without losing unrelated values', () => {
  ...
  try {
    ...
    const settings = { ...defaultSettings, interfaceFont: '', remoteSessionIntegration: false, terminalLigatures: false, appearance: 'system' as const, terminalFont: 'DejaVu Sans Mono', terminalFontSize: 18 };
    ...
    const changed = prepareProfile(catalog, edited, { token: '00000000-0000-4000-8000-000000000000', values: { port: 2222, remote_sessions: true, terminal: { font: '', font_size: 20, ligatures: true } }, reset: [] });
    ...
    assert.deepEqual(loadCatalog(file).profiles[0].spec.terminal, { font: '', font_size: 20, ligatures: true });
    ...
    const reset = prepareProfile(loadCatalog(file), resetDraft, { token: '00000000-0000-4000-8000-000000000000', values: {}, reset: ['remote_sessions', 'terminal.font', 'terminal.font_size', 'terminal.ligatures'] });
