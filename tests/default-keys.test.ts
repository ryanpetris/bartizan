...
test('automatic authentication uses OpenSSH default identity paths unless files are selected', () => {
  const catalog = { defaults: {}, profiles: [], file: '', settings: { appearance: 'dark' as const, theme: 'rail' as const, interfaceFont: 'Inter', terminalLigatures: true, terminalWebgl: true, remoteSessionIntegration: true, terminalFont: '', terminalFontSize: 13 } };
