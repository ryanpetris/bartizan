...
Browser tabs use sandboxed `WebContentsView` instances without a preload or Node integration. The renderer reports the browser area's bounds to the main process, which positions the selected native view. Modal dialogs hide the view so that native page content does not cover app controls.
