...
[Terminals](../src/renderer/terminals.tsx) use xterm with font loading, a fit addon, web links and a ligature joiner. WebGL is preferred; initialization failure or unrecoverable context loss falls back to DOM rendering. Context loss adds a current error for the affected terminal until WebGL recovers or the terminal closes. An initialization failure instead adds one app-wide current error for the page. Graphics errors belong to the page displaying the terminals and appear alongside backend errors in the Errors panel.
